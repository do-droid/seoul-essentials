...
def get_subway_timetable(
    ...
    Covers every station on Seoul Metro lines 1-9 including transfer
    stations on all their lines (e.g. Jongno 3-ga on lines 1, 3 AND 5),
    with full official timetables (Seoul Open Data, ~1,000+ trains/day
    per station).
    ...
    Args:
        ...
        day_type: "weekday", "saturday", or "holiday" (Sundays use "holiday").
        ...
        full_day: True returns the complete day timetable (large; ~40KB per
            line). Prefer `after` for "next train" questions.
    ...
    result = _api_subway(
        ...
        direction=direction, after=after, full_day=full_day,
    )
